...
def recommend_portfolio(intent_request):
    ...
    if 0 < age < 65 and investment_amount >= 5000:
        if risk_level=='None' or risk_level=='none':
            recommendation="100% bonds (AGG)"
        ...
        raise Exception("Invalid age- please try again with a positive age")
    elif age >= 65:
        raise Exception("Invalid age- please try again with an age below 65")
    elif investment_amount<5000:
        raise Exception("Please input an investment amount equal to or greater than $5000")
        
    return(close(intent_request["sessionAttributes"],"Fulfilled",{
        "contentType":"PlainText",
        "content":f"Thank you for your information; We recommend a portfolio of {recommendation}"
    }
    )
    )
